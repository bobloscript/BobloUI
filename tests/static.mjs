...
if(!hasCode(sectionSrc,'Icon=options.Icon') || !sectionSrc.includes('function Section:SetIcon') || !sectionSrc.includes('Name = "SectionHeader"') || !hasCode(build,'local SECTION_KEYS={Id=true,Title=true,Description=true,Icon=true')) fail('section visual identity API missing'); else ok('section icon + structured header wired');
